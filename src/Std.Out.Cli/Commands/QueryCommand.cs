...
namespace Std.Out.Cli.Commands
{
    ...
    public sealed class QueryCommand(
        IOptions<QueryConfig> _config, IStdOut _service, IDisplayService _display
        ) : IQueryCommand
    {
        ...
        public async Task<Response<Either<BadRequest, Unit>>> Execute(CommandModel command)
        {
            ...
            var stdConfig = BuildStdConfig(source.StdOut.Sources);
            ...
            if (query)
            {
                ...
                query.Value.Length.LogValue(x => "{Count} query matches found.".WithArgs(x));
        ...
        private static Response<QuerySourceModel> GetSourceModel(string key, QueryConfig config)
        {
            var response = new Response<QuerySourceModel>();
            var model = new QuerySourceModel();
            ...
            if (source == default) return response.LogErrorValue("Query source [{Key}] not found.".WithArgs(key));
            ...
            {
                isValid.LogErrorValue("The merged Query source values are not valid for key: [{Key}].".WithArgs(key));
            }
        ...
        private static StdConfig BuildStdConfig(StdOutOptionsSources source)
        {
            var config = default(StdConfig);
            var dissect = Operations.Store | Operations.Load;
